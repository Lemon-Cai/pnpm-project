...
// let DEFAULT_CHUNK_SIZE = 1024 * 1024 * 1
  onmessage = async function (evt) {
    ...
    const { fileList = [], chunkSize: DefaultChunkSize } = evt.data

    if (fileList?.length > 0) {
      // 批量生成md5
      const fileMd5List = []
      for (const currentFile of fileList) { 
        const hash = await _generateMD5(currentFile.raw, DefaultChunkSize)
        // console.log(hash, currentFile);
        fileMd5List.push({
          hash,
          uid: currentFile.uid
        })
      ...
      if (messagePort) {
        fileMd5List.length > 0 && messagePort.postMessage(fileMd5List)
      } else {
        postMessage(fileMd5List) // 这里是通知主线程
      }
    }
  }
